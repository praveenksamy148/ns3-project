 ...
 */
 ...
 #include "log-distance-random-propagation-model.h"
 ...
 int main (int argc, char *argv[])
 {
 ...
 
   {
     ...
     Ptr<LogDistanceRandomPropagationModel> gaussianProp = CreateObject<LogDistanceRandomPropagationModel> ();
     gaussianProp->SetAttribute("Exponent", DoubleValue (2.5));
     gaussianProp->SetAttribute("ReferenceDistance", DoubleValue (1.0));
     gaussianProp->SetAttribute("ReferenceLoss", DoubleValue (46.6777));
     gaussianProp->SetAttribute("RandomVariable", StringValue ("ns3::NormalRandomVariable[Mean=0|Variance=2]"));
     
     for (double distance = 200.0; distance <= 500.0; distance += 50.0)
     {
         Gnuplot2dDataset dataset = TestProbabilistic (gaussianProp, distance);
     ...
     plot.SetTitle ("GaussianPropagationLossModel");
